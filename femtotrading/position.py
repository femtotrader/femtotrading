...
class Position(object):
    ...
    def _calculate_initial_value(self):
        ...
        if self.action == "BOT":
            ...
            self.avg_bot = self.init_price.quantize(PLACES[5])
            self.total_bot = (self.buys * self.avg_bot).quantize(PLACES[2])
            self.avg_price = (
            ...
            self.avg_sld = self.init_price.quantize(PLACES[5])
            self.total_sld = (self.sells * self.avg_sld).quantize(PLACES[2])
            self.avg_price = (
                (self.init_price * self.quantity - self.init_commission) / self.quantity
            ).quantize(PLACES[5])
            self.cost_basis = (
        ...
        self.net_total = (self.total_sld - self.total_bot).quantize(PLACES[2])
        self.net_incl_comm = (self.net_total - self.init_commission).quantize(PLACES[2])
    ...
    def transact_shares(self, action, quantity, price, commission):
        ...
        self.net_total = (self.total_sld - self.total_bot).quantize(PLACES[2])
        self.net_incl_comm = (self.net_total - self.total_commission).quantize(PLACES[2])

        # Adjust average price and cost basis
        self.cost_basis = (self.quantity * self.avg_price).quantize(PLACES[2])
